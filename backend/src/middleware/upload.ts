...
import { SystemSettingsCache } from '../services/system-settings-cache.service';

// Env-var fallback for file size (used when DB is unavailable)
const envMaxFileSizeMB = parseInt(process.env.MAX_FILE_SIZE_MB || '15', 10);
...
// Static multer instance (uses env var — kept for direct import compatibility)
export const multerUpload = multer({
  storage,
  limits: { fileSize: envMaxFileSizeMB * 1024 * 1024, files: 1 },
  fileFilter,
});

/**
 * Validates magic bytes of file buffer.
 */
export const validateMagicBytes = (buffer: Buffer, claimMime: string): boolean => {
  ...
  if (claimMime === 'text/plain') return true;
  return false;
};

/**
 * Dynamic upload middleware — reads max_file_size_mb from DB settings cache per request.
 */
export const handleSingleFileUpload = (fieldName: string) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    const storageSettings = await SystemSettingsCache.getStorage().catch(() => null);
    const maxFileSizeMB = storageSettings?.max_file_size_mb ?? envMaxFileSizeMB;
    ...
    const uploadSingle = dynamicUpload.single(fieldName);

    uploadSingle(req, res, (err: any) => {
      if (err instanceof multer.MulterError) {
        if (err.code === 'LIMIT_FILE_SIZE') {
          return sendError(res, 400, `File size exceeds maximum limit of ${maxFileSizeMB} MB.`);
        }
